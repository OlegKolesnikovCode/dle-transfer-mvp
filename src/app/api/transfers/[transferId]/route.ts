...
import { deriveTransferById } from "../../../../server/ledger/read-derivation-boundary";
 ...
 * - TEST-011
 *
 * This route provides derived Transfer read state only.
...
export async function GET(
  ...
  const derived = await deriveTransferById(transferId);
  ...
  const response = createSuccessResponse({
    pairing: {
      transferId: derived.transfer.id
    },
    result: {
